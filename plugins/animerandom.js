...
let cita = arr[Math.floor(Math.random() * arr.length)]
  await conn.sendBI(m.chat, `  ආ 🥱${command}nya @${m.sender.split('@')[0]}`, wm, cita, [[`Next`, `${usedPrefix}${command}`]], m, {mentions: [m.sender], jpegThumbnail: await(await fetch(cita)).buffer()})
}
handler.tags = ['anime']
